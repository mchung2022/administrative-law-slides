...
def generate_1000_word_script(slide):
    ...
    # Question type tag enhancement
    q_type_str = ""
    stype = slide.get('type', 'concept')
    if stype == 'matching':
        q_type_str = "【觀念對應與連連看實務演練】"
    elif stype == 'multiple_choice':
        q_type_str = "【素養導向選擇題應試解析】"
    elif stype == 'short_answer':
        q_type_str = "【觀念簡答與申論題解題剖析】"
    elif stype == 'true_false':
        q_type_str = "【觀念是非判斷與法理辨析】"
    elif stype == 'fill_in_blank':
        q_type_str = "【法條填空與關鍵字精準記憶】"
    
    sec1 = f"📌 【一、本頁核心主題與法理立論】：各位同學們好！歡迎來到第 {sid} 頁《{title}》的深度講堂。本頁歸屬於【{mod}】單元，類別定位為「{category}」{q_type_str}。在現代法治國家的核心體系中，行政法並非單純嚴苛的官僚管制條文，而是國家保障人民基本權利、約束公權力恣意行使的黃金防線。本頁的核心學習目標，在於引導大家建立對「{topic}」的深刻法理直覺。行政機關在日常推動各項公共建設、維護社會秩序或執行行政處分時，無時無刻不受到依法行政原則、法律保留原則與比例原則的嚴格約束。希望大家在閱讀本頁內容時，能牢記：法律不是高深莫測的條文，而是維持自由與正義的基石！"
