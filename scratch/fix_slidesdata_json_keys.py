...
print("=== Forcing podcastScript on ALL 500 slides in js/slidesData.js ===")
...
for slide in slides:
    ...
    if 'podcastScript' not in slide or not slide['podcastScript'] or len(str(slide['podcastScript'])) < 20:
        slide['podcastScript'] = script_text
        filled_count += 1

print(f"Filled missing podcastScript for {filled_count} slides.")

with open(slides_path, 'w', encoding='utf8') as f:
    f.write('window.slidesData = ' + json.dumps(slides, ensure_ascii=False, indent=2) + ';\n')

print(f"[OK] Saved 500 slides to js/slidesData.js with 100% podcastScript coverage!")
