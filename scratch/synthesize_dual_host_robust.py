...
print("=== Robust Dual-Host Speech Synthesis (Natural Adult Speed: Rate=-15%) ===")

json_path = os.path.join(os.path.dirname(__file__), 'podcast_script_30min.json')
output_dir = os.path.join(os.path.dirname(__file__), 'ch_audio_dual_natural')
...
async def main():
    ...
    for ch in chapters:
        ...
        for l_idx, line in enumerate(lines):
            ...
            ok = await speak_with_retry(clean_text, voice, line_mp3, rate="-15%")
            if not ok:
                print(f"❌ Failed to synthesize line {l_idx+1} in Chapter {cid}")
