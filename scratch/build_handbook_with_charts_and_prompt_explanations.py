...
print("=== Generating Master Handbook with Embedded Matplotlib Charts & Prompt Explanations ===")
...
add_callout(
    ...
    "特別升級版：手冊中不僅包含全部 7 大階段完整 Prompt 提示詞模板，更針對每一個 Prompt 參數的背後意義、設計哲學與 AI 機制進行深度解說，並內嵌由 Python Matplotlib 實測繪製之架構圖與數據圖表！",
    type_style='note'
)
...
# =========================================================================
# CHAPTER 3 - PROMPT TEMPLATE 1 & DETAILED EXPLANATION
# =========================================================================
add_h1("第三章：500 頁 HTML 互動簡報系統架構與開發實戰")

add_h2("3.1 簡報系統架構與提示詞模板 1")
...
add_h2("3.2 Prompt 模板 1 具體意義與參數運算剖析")
add_p("為什麼 Prompt 模板 1 能夠一次引導 AI 產出 500 頁高階簡報？其背後的指令設計邏輯如下：")
add_bullet("【角色設定為 Web 前端架構師】：強制 AI 採用『資料驅動（Data-driven）』架構，將 500 頁簡報內容抽取至 slidesData.js，避免把 500 頁 HTML 硬寫在單一檔內導致瀏覽器卡頓。", bold_prefix="解說 1（資料與 UI 分離）：")
add_bullet("【指定深色極光 (Dark Aurora) 與 HSL 配色】：明確交代視覺風格，防範 AI 使用瀏覽器預設的白底黑字或平庸配色，確保第一眼呈現企業級奢華感。", bold_prefix="解說 2（視覺美學約束）：")
add_bullet("【要求 3 卡視覺容器】：引導 AI 在處理 80+ 頁新聞案例時，自動生成三欄式 Flex/Grid 卡片布局，精準對應【事件脈絡】、【法律爭點】與【大考考點】。", bold_prefix="解說 3（組件結構化）：")
add_bullet("【要求 build_single_file.js 打包腳本】：指示 AI 編寫 Node.js 打包工具，自動將 external CSS 與 JS 壓縮編譯為獨立單檔，方便學員離線分享。", bold_prefix="解說 4（自動化打包）：")

doc.add_page_break()

# =========================================================================
# CHAPTER 4 - PROMPT TEMPLATE 2 & DETAILED EXPLANATION
# =========================================================================
add_h1("第四章：50 萬字大書講稿與 Word 全集自動化生成")

add_h2("4.1 講稿生成與提示詞模板 2")
...
add_h2("4.2 Prompt 模板 2 具體意義與參數運算剖析")
add_bullet("【指定法學教授身份與條文清單】：引導 AI 在撰寫延伸講稿時，主動調用《行政程序法》與大法官字號，避免生成空泛無法律依據的套話。", bold_prefix="解說 1（法律精確度）：")
add_bullet("【指定 python-docx 與顏色碼 (#1B365D)】：精確指示排版庫與 HEX 顏色碼，讓 AI 自動調用 parse_xml 控制 Word 表格背景色與標題字型，實現免手動調整的企業級排版。", bold_prefix="解說 2（程式排版自動化）：")
add_bullet("【要求 100 輪法律稽核腳本】：強迫 AI 建立 verify_100_rounds_docx_taiwan_law.py 驗證腳本，自我審查有無誤用外國法規或條號錯誤，確保內容 100% 正確。", bold_prefix="解說 3（客觀驗證閉環）：")

doc.add_page_break()

# =========================================================================
# CHAPTER 5 - PROMPT TEMPLATE 3 & DETAILED EXPLANATION
# =========================================================================
add_h1("第五章：30 分鐘特企 Podcast 廣播對談劇本寫作與聲學設計")

add_h2("5.1 劇本設計與提示詞模板 3")
...
add_callout(
    ...
    "【要求】：\n"
    "1. 節目架構：涵蓋行政法 10 大核心模組，每單元對談約 3 點鐘。\n"
    "2. JSON 格式：每一句對白需包含 speaker (阿哲/小晨)、voice (zh-TW-YunJheNeural/zh-TW-HsiaoChenNeural) 與 text 欄位。",
    type_style='prompt'
)

add_h2("5.2 Prompt 模板 3 具體意義與參數運算剖析")
add_bullet("【雙主持角色分工 (YunJhe vs HsiaoChen)】：建立男聲（提問者）與女聲（解答者）的性格對比，使節目具備廣播電台般的生動節奏，完全擺脫單人朗讀的枯燥感。", bold_prefix="解說 1（雙人對話機制）：")
add_bullet("【限定 13,000 字與 10 大模組】：根據廣播語速精算字數，確保 10 大模組均勻分配對話時長，精確達到 30 分鐘廣播節目規格。", bold_prefix="解說 2（時長與字數精算）：")
add_bullet("【指定包含 voice 欄位的 JSON 格式】：直接將 Azure Neural 語音模型的代號寫入 JSON 欄位，方便後續 Python 腳本直接讀取並調用 TTS 引擎。", bold_prefix="解說 3（聲學數據對接）：")

doc.add_page_break()

# =========================================================================
# CHAPTER 6 - PROMPT TEMPLATE 4 & DETAILED EXPLANATION
# =========================================================================
add_h1("第六章：Microsoft Azure Neural 神經語音 TTS 語音合成與語速校準")

add_h2("6.1 語速實測與提示詞模板 4")
...
add_h2("6.2 Prompt 模板 4 具體意義與參數運算剖析")
add_bullet("【指定實測 rate=-15% (4.22 字/秒)】：防范 edge-tts 預設語速過快或設定過慢（-50%）導致失真，以實測數據導向確定最舒適的成年人對談語速。", bold_prefix="解說 1（語速精準控制）：")
add_bullet("【要求 try...except 與 0.3s 緩衝】：指示 AI 在存取微軟 TTS 伺服器時寫入防護邏輯，避免因 WebSocket 連線過密引發 NoAudioReceived 錯誤而中斷。", bold_prefix="解說 2（連線穩定防護）：")
add_bullet("【要求撰寫 MP3 Frame Header 解析器】：引導 AI 以純 Python 解析 MP3 音訊標頭，精算出各章節起訖秒數，實現 100% 精確的時間軸字幕同步。", bold_prefix="解說 3（時間軸毫秒計算）：")

doc.add_page_break()

# =========================================================================
# CHAPTER 7 - PROMPT TEMPLATE 5 & DETAILED EXPLANATION
# =========================================================================
add_h1("第七章：雙引擎網頁版 Podcast 播放器 (podcast_player.html) 開發")

add_h2("7.1 播放器設計與提示詞模板 5")
...
add_h2("7.2 Prompt 模板 5 具體意義與參數運算剖析")
add_bullet("【雙引擎備援機制】：解決瀏覽器在本地 file:/// 開啟實體 MP3 時可能的 CORS 限制，確保在任何環境下皆能正常發聲。", bold_prefix="解說 1（雙引擎容錯）：")
add_bullet("【Canvas 頻譜視覺化】：指示 AI 使用 HTML5 Canvas API 解析音訊頻率，畫出高階電台質感的波形動畫，顯著提升使用者體驗。", bold_prefix="解說 2（動態視覺體驗）：")
add_bullet("【雙向時間軸毫秒級同步】：利用 currentTime 事件監聽與 CSS scrollTop 控制，實現點擊章節即時跳轉與逐字稿自動追蹤高亮。", bold_prefix="解說 3（播放同步邏輯）：")

doc.add_page_break()

# =========================================================================
# CHAPTER 8 - PROMPT TEMPLATE 6 & DETAILED EXPLANATION
# =========================================================================
add_h1("第八章：100 輪與 50 輪自動化測試驗證與品質監控")

add_h2("8.1 測試架構與提示詞模板 6")
...
add_h2("8.2 Prompt 模板 6 具體意義與參數運算剖析")
add_bullet("【強迫寫入客觀稽核腳本】：打破一般使用者僅靠肉眼檢查的盲點，透過 Python/Node 腳本進行 100 輪量化檢驗，確保完全零死角。", bold_prefix="解說 1（量化客觀驗證）：")
add_bullet("【Node.js vm DOM 沙盒技術】：引導 AI 利用 Node 的 vm 模組在記憶體中模擬瀏覽器事件，無需開啟實體瀏覽器即可在秒級內完成 50 次點擊測試。", bold_prefix="解說 2（無頭極速測試）：")
add_bullet("【自動修復閉環 (Auto-Repair Loop)】：當測試腳本回傳 Fail 時，Antigravity AI 會讀取報錯訊息（Stack Trace）並自動修復代碼，直到通過率達到 100%！", bold_prefix="解說 3（自動修復機制）：")

doc.add_page_break()

# =========================================================================
# CHAPTER 9 - PROMPT TEMPLATE 7 & DETAILED EXPLANATION
# =========================================================================
add_h1("第九章：GitHub Pages 線上部署與 Git 版本管理實戰")

add_h2("9.1 Git 部署與提示詞模板 7")
...
add_h2("9.2 Prompt 模板 7 具體意義與參數運算剖析")
add_bullet("【使用 git add -f 強制追蹤大檔案】：解決 MP3 音訊檔常因 .gitignore 規則被意外忽略的問題，確保音訊資源順暢上傳至 GitHub。", bold_prefix="解說 1（大檔案追蹤技巧）：")
add_bullet("【GitHub Pages 一鍵發布】：指導 AI 給出正確的 Settings 部署指引，讓學員迅速取得免費且具備 HTTPS 加密憑證的全網可存取網址。", bold_prefix="解說 2（免費雲端託管）：")
...
add_callout(
    ...
    "恭喜您完整研讀本講義！您已掌握與 Antigravity AI 協作開發 500 頁 HTML 簡報、講稿全集與 30 分鐘廣播 Podcast 的全部核心技術、7 大階段完整 Prompt 提示詞指令與參數運算解說。快去開啟您的下一個 AI 協作專案吧！",
    type_style='tip'
)

# Save Document
doc_out_path = os.path.join(os.path.dirname(__file__), '..', 'Antigravity_AI全方位實戰指南_HTML簡報與網頁Podcast產製講義_完整Prompt解說版.docx')
doc.save(doc_out_path)
print(f"[OK] Master Handbook with Prompt Explanations & Matplotlib Charts successfully saved to: {doc_out_path}")
