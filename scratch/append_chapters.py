...
print("=== Appending 10 Detailed Chapters to build_master_handbook.py ===")

script_path = os.path.join(os.path.dirname(__file__), 'build_master_handbook.py')
...
add_callout(
    "講義使用說明與核心目標",
    "本講義專為 Antigravity AI 人工智慧協作系統的初學者所編寫。全書以「行政法 500 頁旗艦簡報與 30 分鐘雙主持廣播 Podcast」專案為真實範例，手把手引導學員掌握如何透過自然語言與 Antigravity AI 進行雙人對話（Pair Programming）、需求定義、自動化程式碼編寫、音訊合成、自動化測試與 GitHub 雲端部署。讀完本講義後，您將具備獨自引導 AI 產出企業級大型 Web 應用與多媒體系統的完整能力！",
    type_style='note'
)

add_custom_table(
    ["模組指標", "專案規格與達成成就", "技術說明與產出"],
    [
        ["HTML 簡報系統", "500 頁全動態互動簡報 (1.35 MB)", "資料驅動架構、5 大題型、80+ 臺灣新聞案例3卡解析"],
        ["Word 講稿全集", "50 萬字大書 Word 輸出 (.docx)", "單頁 1,000 字補充、內嵌 Matplotlib 統計圖表、企業排版"],
        ["廣播 Podcast", "30 分鐘雙主持對談節目 (9.18 MB)", "Azure Neural 雙神經語音、成年人自然語速 (-15%)、10 大章節"],
        ["網頁播放器", "雙引擎雙主持音訊播放器", "HTML5 MP3 + Web Speech TTS 備援、Canvas 動態頻譜、毫秒同步"],
        ["品質驗證", "100 輪法律稽核 + 50 輪 UI 測試", "100/100 法條判例交叉驗證通過、50/50 時間軸同步點擊驗證"]
    ]
)
...
add_h2("1.1 什麼是 Antigravity AI？進階代理人（Agentic Coding）機制")
...
add_p("在與 Antigravity 協作時，AI 不僅是程式碼生成器，更是您的數位資深架構師與 Pair Programming 夥伴。它能夠直接存取專案檔案系統、執行 Python 與 Node.js 命令、編寫自動化測試腳本、以及管理背景任務。")
...
print(f"[OK] Master Handbook successfully built and saved to: {doc_out_path}")
'''

with open(script_path, 'a', encoding='utf8') as f:
    f.write(chapter_code)

print("[OK] Successfully appended chapters to build_master_handbook.py!")
